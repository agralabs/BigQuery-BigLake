...
uris = ['gs://[YOUR_GCS_BUCKET]/[YOUR_ICEBERG_DATA_FOLDER]/']
...
Open Airflow UI: Navigate to the DAG banking_pipeline_soft_delete.

  

Go to Grid/Graph View: Click on the task that failed (e.g., extract_mysql_to_gcs).

  

Clear the Task: Click the Clear button.

  

Include Downstream: Make sure the Downstream option is checked so that all subsequent tasks (DTS, Upsert) are also reset.

  

Confirm: Click OK / Clear.
...
The DTS Cloud Storage URI dynamically adds 7 hours to its UTC runtime: export_{run_time+7h|"%Y%m%d"}. This perfectly aligns the DTS search query with Airflow's output folder.# End-to-End Data Pipeline: MySQL to BigQuery (Iceberg Staging & Native Upsert)
